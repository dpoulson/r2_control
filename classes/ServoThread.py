  ...
  def run(self):
     ...
     while True:
       ...
       if duration > 0:
         ...
         if __debug__:
            print "Ticks:%s  Current Position: %s Position shift: %s Starting Position: %s End Position %s" % (ticks, self.Current, tick_position_shift, tick_actual_position, actual_position)
         ...
         if __debug__:
            print "Setting servo %s(%s) to position = %s(%s)" % ("test", self.Channel, actual_position, position)
